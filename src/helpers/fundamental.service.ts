...
// import FundamentalData from '../models/stockFundamental.js';
...
export const fetchFundamental = async (symbol: string) => {
    const YF = new YahooFinance();
    try {
        const data = await YF.fundamentalsTimeSeries(
            symbol,
            {
                period1: '2024-01-01',
                type: 'annual',
        ...
        const latestData = data.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0] as FundamentalData;
...
export const getLatestBalanceSheet = (data: any) => {
    return data?.balanceSheetHistory?.balanceSheetStatements?.[0];
}

export const calculateRatios = (financials: any) => {
...
export const updateFundamentals = async (symbol: string) => {
